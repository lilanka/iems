...
def cpo_step(env_name, policy_net, value_net, states, actions, returns, advantages, cost_advantages, constraint_value, d_k, max_kl, damping, l2_reg, use_fim=True):
    ...
    flat_params, _, opt_info = scipy.optimize.fmin_tnc(get_value_loss, get_flat_params_from(value_net).detach().cpu().numpy())
    ...
    d_k = to_tensor(d_k).to(constraint_value.dtype).to(constraint_value.device)
